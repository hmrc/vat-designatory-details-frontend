...
class LanguageSwitchController @Inject()(configuration: Configuration,
  ...
  private def langToCall: String => Call = appConfig.routeToSwitchLanguage

  private def fallbackURL: String = routes.HelloWorldController.show().url

  private def languageMap: Map[String, Lang] = appConfig.languageMap

  def switchToLanguage(language: String): Action[AnyContent] = Action {
    implicit request =>
      val enabled = appConfig.features.languageToggleEnabled()
      val lang = if (enabled) {
